...
description: Create distinctive, high-converting landing pages that combine proven conversion elements with exceptional design quality. Build beautiful, memorable landing pages using Astro and Tailwind CSS that avoid generic AI aesthetics while following the 11 essential elements framework.
---
...
- **Standard HTML/CSS components** (encapsulated in Astro components)
- **View Transitions** (optional) for smooth navigation

### UI Component Strategy

Instead of relying on heavy component libraries, build lightweight, accessible components directly in Astro:

- Create reusable UI parts in `src/components/ui/`
- Use Tailwind for all styling
- Utilize Astro's `<slot />` for flexible content projection
- Keep client-side JavaScript to a minimum (Island Architecture)

### Why Astro?
- **Performance**: Zero JS by default, perfect for landing pages
- **Simplicity**: HTML-first approach with scoped styles
- **Flexibility**: Use any framework components if absolutely needed (but prefer native Astro)
- **Asset Optimization**: Built-in image and font optimization
...
│   │   ├── FAQ.astro           # FAQ Accordion (Element 9)
...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'OG Title',src/layouts/Layout.astro`.
Use the definition of the `Props` interface to allow passing the title and description from the pages.
...
</html>. **Benefits** - Asymmetric layout, custom icons, animated on scroll
...
Map sections to Astro components and **customize heavily**:

**Hero CTA Example (Hero.astro):**
```astro
---
const { href, text = "Get Started" } = Astro.props;
---
<a
  href={href}
  class="bg-accent hover:bg-accent/90 text-white px-12 py-6 text-xl font-display rounded-full shadow-2xl hover:shadow-accent/50 hover:scale-105 transition-all duration-300 inline-block"
>
  {text} &rarr;
</a>
```

**Benefits Card Example (Benefits.astro):**
```astro
---
// Component logic here if needed
---
<div class="border-2 border-primary/10 hover:border-primary/30 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-gradient-to-br from-white to-primary/5 rounded-xl p-6">
  <slot />
</divCustom content */}
</Castro
---
...
<style>
/* Scoped styles in Astro, or put this in global.css */
@keyframes fade-in {
  from {
    opacity: 0;
...
For scroll animations, use Intersection Observer or native View Transitions
Astro's `<Image />` component for all images
- Add `loading="eager"` to above-the-fold images (hero section)
- Implement lazy loading for below-the-fold content (default in Astro Image)
- Optimize fonts: Use valid web fonts and font-display: swap
- Use CSS animations over JavaScript when possible
- Keep the number of client-side scripts to a minimum (`client:*` directives)
...
For complete component implementations, refer to the Astro components in the project structure.
...
- [ ] **Astro Components**: Built cleanly with standard HTML/Tailwind
...
- [ ] Astro 4+
- [ ] TypeScript types defined in Frontmatter
- [ ] Tailwind CSS styling
- [ ] Components properly scoped and reusable
- [ ] Metadata configured for SEO (in Layout)
- [ ] Images optimized with Astro Image component
- [ ] Responsive design implemented (mobile-first)
- [ ] Accessibility standards met (WCAG AA)
- [ ] Performance optimized (minimal client-side JS)
...
5. **Build Custom**: Rely on simple HTML/CSS over heavy libraries when possible
